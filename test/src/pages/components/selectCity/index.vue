    ...
    mounted() {
        this.$nextTick(function() {
            this.$api.vkcPost("supermarketloan/other/cityinfohome", { param: { ip: returnCitySN.cip } }).then(res => {
                ...
                this.currentCity.cityCode = res.currentCityCode;
                this.citys = res;
                $(".mint-indexlist-content").height($(".mint-indexlist-content").height() - 50); // jq计算高度
                // let content = document.getElementsByClassName("mint-indexlist-content")[0];   // 原生计算高度
                // content.style.height = parseFloat(content.style.height) - 50 + "px";
            });
        });
    }
};
...
.mint-indexsection .mint-cell:last-child {
